function showContent(section) {
    ...
    `;
        ...
        sectionContent = `
            <h2>자기소개서</h2>
            <p>
             서비스의 완성도는 개발이 아닌 검증에서 결정된다고 생각합니다. 저는 사용자 입장에서 문제를 발견하고 서비스 품질을 높이는 과정에 흥미를 느껴 QA 직무를 선택하게 되었고, 현재 관련 업무를 수행하며 그 중요성을 직접 경험하고 있습니다.         
            </p>
            <p>
             실무에서 웹 기반 시스템의 기능 검증과 결함 관리를 담당하며 다양한 유형의 오류를 확인해왔습니다. 단순히 기능이 동작하는지 확인하는 것을 넘어 실제 사용자 환경에서 발생할 수 있는 문제를 중심으로 테스트를 진행하였고, 그 과정에서 UI와 데이터가 일치하지 않는 문제, 삭제된 정보가 즉시 반영되지 않는 동기화 오류, 사용자 권한과 기능 접근이 맞지 않는 문제 등을 발견한 경험이 있습니다.            
            </p>
            <p>
            특히 테스트를 수행하면서 가장 크게 느낀 점은, 예상했던 결함보다 생각하지 못했던 부분에서 문제가 발생할 때가 많다는 점이었습니다. 이러한 예상 밖의 오류를 발견하고 원인을 추적하는 과정에서 QA 업무의 재미를 느끼게 되었고, 단순한 확인 작업이 아니라 서비스의 완성도를 높이는 중요한 역할이라는 확신을 가지게 되었습니다.
            </p>
            <p>
            장비 삭제 이후에도 기존 연결 정보가 유지되는 오류나, 이미지 변경 후 새로고침을 해야만 반영되는 UI 문제 역시 이러한 관점에서 발견한 사례입니다. 사용자 입장에서 혼란을 줄 수 있는 요소라고 판단하여 중점적으로 검증하였고, 재현 절차와 영향 범위를 포함한 결함 리포트를 작성하여 개발자와의 협업을 통해 개선까지 이어질 수 있도록 했습니다.
            </p>
            <p>
            또한 Postman을 활용한 API 응답 검증과 JMeter를 통한 간단한 성능 테스트를 수행하며 기능 테스트를 넘어 시스템 관점에서도 품질을 확인하려고 노력했습니다. 테스트 케이스를 직접 설계하고 입력값 검증, 예외 처리, 경계값 테스트를 수행하면서 보다 체계적인 QA 방식에 대해 고민하게 되었습니다.
            </p>
            <p>
            이전에는 AI 기반 화재 감지 프로젝트를 진행하며 데이터와 결과의 정확도를 검증하는 경험을 했고, 이를 통해 ‘동작하는 것’과 ‘정확하게 동작하는 것’의 차이를 이해하게 되었습니다. 이러한 경험은 현재 QA 업무를 수행하는 데 있어 문제를 더 깊이 있게 바라보는 데 도움이 되고 있습니다.
            </p>
            <p>
            저는 단순히 테스트를 수행하는 것을 넘어, 예상하지 못한 문제까지 찾아내고 그 원인을 분석하는 QA가 되고자 합니다. 앞으로는 기능 테스트를 넘어 성능, 보안, 자동화 테스트까지 역량을 확장하여 서비스 품질을 전반적으로 책임질 수 있는 QA 엔지니어로 성장하겠습니다.
            </p>
        `;
